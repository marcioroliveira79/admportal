...
$titulo_listar="Menus Já Cadastrados";
$id_item=$_GET['id_item'];
$arquivo="cad_menu.php";
$tabela="sgc_menu";
$id_chave="id_menu";
...
<?
 ...
 echo $permissao_item=acesso($idusuario,$id_item);
  ...
  if($permissao_item=="OK"){
     ...
     //------------------------Deleta na regra de template todos itens que são deste menu---------------------------//
     $deleta = mysql_query("DELETE FROM sgc_template_regra where id_menu=$id_objeto") or print(mysql_error());
     header("Location: ?action=$arquivo&id_item=$id_item");
   }else{
     $msg="Você não tem permissão para esta operação";
     header("Location: ?action=$arquivo&id_item=$id_item&msg=$msg");
