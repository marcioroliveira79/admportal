...
$titulo="Alterar Status Chamados em Grupo";

$id_item=$_GET['id_item'];
$arquivo="alterar_chamado_lote.php";
					...
					<table border="0" width="80%" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td height="23" align="right">
												...
												Id Chamado:&nbsp;&nbsp; </td>
							<td height="23">
										<font size="1"> <select size="1" name="operador" Onchange="atualiza(this.value);" style="font-family: Verdana, arial, helvetica, sans-serif; font-size: 11px; color: #000000;  background: #FFFFFF;"  >
                               <option></option>
                              <option>igual</option>
								<option>maior</option>

						</select>
												<input type="text" name="id_inicio"   size="6" style="background-color: #FFFFFF" maxlength="6">
										</font>à <font size="1">
												<input type="text" name="id_final"  size="6" style="background-color: #FFFFFF" maxlength="6"></td>
							<td width="31">&nbsp;</td>
						</tr>
						<tr>
							<td width="15">&nbsp;</td>
              ...
              <?
                       ...
                       ?>
							...
							<tr>
								...
								<p align="center">Alterar Para</td>
								<td width="31">&nbsp;</td>
							</tr>
							...
							<tr>
								...
								<td height="23">
										<font size="1">
							  <select size="1" name="situacao_alter" style="font-family: Verdana, arial, helvetica, sans-serif; font-size: 11px; color: #000000;  background: #FFFFFF;"  >&nbsp;
                              <option value="Todos">Todos</option>
                              <option value="Fechado">Fechado</option>
...
}elseif($acao_int=="alterar"){

$new_status=$_POST['new_status'];

if ( !empty($_POST['Gravar']) ):
	$array = unserialize(urldecode($_POST['chamados']));
endif;

$count=0;
foreach ($array as $value) {
    $count++;
    $id_historico=tabelainfo($value,"sgc_chamado","id_linha_historico","id_chamado","");
    $value;
    echo "Chamado: $value  - Alterada Situação Para: $new_status<BR>";
    $cadas = mysql_query ("UPDATE sgc_historico_chamado SET situacao = '$new_status',acao = '$new_status' WHERE id_historico = $id_historico");
    $cadas = mysql_query ("UPDATE sgc_chamado SET status = '$new_status' WHERE id_chamado = $value");
}

echo "Total de chamados alterados: $count";
...
if($id_inicio != "" && $id_final != ""){
    $sql_adendo.= " and ch.id_chamado BETWEEN $id_inicio and $id_final ";
}elseif ($id_inicio != "" && $id_final == ""){
    $sql_adendo.= " and ch.id_chamado $op $id_inicio  ";
}elseif ($id_inicio == "" && $id_final != ""){
    $sql_adendo.= " and ch.id_chamado <= $id_final  ";
}
...
while($dados=mysql_fetch_array($checa)){
 ...
 $id_linha_historico       = $dados['ch.id_linha_historico'];
 $count++;

if($situacao_alter!='Todos'){
  $chamados_alterar[]= $id_chamado;
}
...
if ( !empty($_POST['Gravar']) ):
	echo("Dentro do post\n");
	$chamados = unserialize(urldecode($_POST['chamados']));
endif;
...
<?
if($situacao_alter!='Todos' &&  $count>0){
?>
<div align="center">
	<form method="POST" id="form1" name='meuFormulario' enctype="multipart/form-data"  action="?action=<?echo $arquivo?>&acao_int=alterar">
	<input type='hidden' name='id_item' value='<?echo $id_item?>'>
	<input type='hidden' name='new_status' value='<?echo $situacao_alter?>'>
	<table class="border" cellSpacing="0" cellPadding="0" width="200" border="0">
		<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
			<td style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
			<table cellSpacing="1" cellPadding="5" width="200" border="0">
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
				...
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
					<td class="cat" align="center">
                   	Você deseja alterar as informações dos chamados encontrados?<p>
                   	<input type="submit" value="Continuar" name="B1">
                   	<input type="hidden" name="chamados" id="chamados" value="<?=urlencode(serialize($chamados_alterar))?>">
    				<input type="hidden" name="Gravar" id="Gravar" value="Gravar">
