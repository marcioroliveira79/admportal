...
list($men,$usuario) = split('[/]', $post_menu);
     ...
     $checa = mysql_query("select
                           ...
                           (select id_item from sgc_template_regra where  id_template=$usuario)
        ...
        <input type='button' name='botaoEY' onClick='moveElementoDaLista(this.form.selecionados,this.form.todos)' value='<<'>
                          ...
                          $checa = mysql_query("select
                                 ...
                                 ,im.descricao
                           from
                           sgc_template_regra rm
                          ,sgc_item_menu im
                           where
                           rm.id_template = $usuario
                           and rm.id_menu = $men
                           and im.id_item_menu = rm.id_item ") or print(mysql_error());
                           while($dados=mysql_fetch_array($checa)){
                           $id_menu = $dados['id_menu'];
                           $id_item_menu = $dados['id_item_menu'];
                           $descricao = $dados['descricao'];
