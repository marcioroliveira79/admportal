...
$titulo="Cadastro SLA Analísta/Usuário";
$titulo_listar="SLA´s Analísta/Usuário Cadastrados";
...
<form method="POST" name="form1" action="sgc.php?action=cad_sla_analista.php&acao_int=cad_sla_service" onSubmit="return valida_dados(this)">
<div align="center">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" border="0">
				...
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
					...
					<table border="0" width="488" cellspacing="0" cellpadding="0">
                        ...
                        <?
                          $checa = mysql_query("select * from sgc_sla_analista_usuario order by tempo desc ") or print(mysql_error());
                                    while($dados=mysql_fetch_array($checa)){
                                    $id_service = $dados['id_sla_analista'];
                        ...
                        <tr>
							...
							<p align="center"><a href="?action=cad_sla_analista.php&acao_int=editar&id_sla_analista=<?echo $id_service?>&id_item=<?echo $id_item?>"">
							<font color="#000000">Editar</font></a></td>
							<td width="44" height="23">
							<p align="center">
                            <a href="javascript:confirmaExclusao('?action=cad_sla_analista.php&acao_int=excluir&id_sla_analista=<?echo $id_service?>&id_item=<?echo $id_item?>')">
...
echo     $id_sla_analista=$_POST['id_sla_analista'];
...
echo     $desc_sla=$_POST['desc_sla'];
   ...
   if($permissao_item=="OK"){
     ...
     echo  $existe=integridade("$desc_sla","sgc_sla_analista_usuario","descricao","descricao","and id_sla_analista !=$id_sla_analista");
    if($existe=="Existe"){
    ...
    header("Location: ?action=cad_sla_analista.php&acao_int=editar&id_sla_analista=$id_sla_analista&id_item=$id_item&msg=$msg&desc_sla=$desc_sla&ajuda=$ajuda");

    }else{


       $cadas = mysql_query("UPDATE sgc_sla_analista_usuario SET descricao='$desc_sla',ajuda='$ajuda',tempo='$tempo_resp',tipo_tempo='$tipo_tempo',data_alteracao=sysdate(),quem_alterou=$idusuario,oque_alterou='DESC OU TEMPO OU AJUDA' where id_sla_analista='$id_sla_analista'") or print(mysql_error());
       session_unregister('ajuda');
       session_unregister('desc_sla');
       session_unregister('tempo_resp');
       session_unregister('tipo_tempo');
     header("Location: ?action=cad_sla_analista.php&id_item=$id_item");
     ...
     session_unregister('tempo_resp');
     session_unregister('tipo_tempo');
     header("Location: ?action=cad_sla_analista.php&id_item=$id_item&msg=$msg");
   }
...
$id_sla_analista=$_GET['id_sla_analista'];
$id_item=$_GET['id_item'];

$checa = mysql_query("select * from sgc_sla_analista_usuario where id_sla_analista=$id_sla_analista ") or print(mysql_error());
                                    while($dados=mysql_fetch_array($checa)){
                                    $ler_descricao = $dados['descricao'];
                                    $ler_ajuda = $dados['ajuda'];
                                    $ler_tempo = $dados['tempo'];
...
<form method="POST" name="form1" action="sgc.php?action=cad_sla_analista.php&acao_int=editar_bd"" onSubmit="return valida_dados(this)">
<div align="center">
	<table class="border" cellSpacing="0" cellPadding="0" width="100%" border="0">
		<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
			<td style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
				...
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
					...
					<table border="0" width="587" cellspacing="0" cellpadding="0">
						<tr>
							...
							<p align="center"><font color="#FF0000"><?echo $msg?></font></td>
                            <input type='hidden' name='id_item' value='<?echo $id_item?>'></td>
                            <input type='hidden' name='id_sla_analista' value='<?echo $id_sla_analista?>'></td>
						</tr>
                        <?
...
<?
 ...
 $idusuario = $_SESSION['id_usuario_global'];
 $id_item=$_GET['id_item'];
 echo $id_sla_service=$_GET['id_sla_analista'];

 echo $permissao_item=acesso($idusuario,$id_item);
  
  if($permissao_item=="OK"){
     $deleta = mysql_query("DELETE FROM sgc_sla_analista_usuario where id_sla_analista=$id_sla_service") or print(mysql_error());
     header("Location: ?action=cad_sla_analista.php&id_item=$id_item");
   }else{
     $msg="Você não tem permissão para esta operação";
     header("Location: ?action=cad_sla_analista.php&id_item=$id_item&msg=$msg");
       ...
       $integridade=integridade($desc_sla,"sgc_sla_analista_usuario","descricao","descricao");


  
    if($integridade=="Existe"){

      header("Location: ?action=cad_sla_analista.php&id_item=$id_item&msg=Já existe um SLA com esse nome");
  
    }else{

     $cadas = mysql_query("INSERT INTO sgc_sla_analista_usuario (descricao, ajuda, tempo, tipo_tempo, data_criacao, quem_criou) VALUES ('$desc_sla','$ajuda','$tempo_resp','$tipo_tempo',sysdate(),$idusuario)") or print(mysql_error());
      session_unregister('ajuda');
      session_unregister('desc_sla');
      session_unregister('tempo_resp');
      session_unregister('tipo_tempo');
