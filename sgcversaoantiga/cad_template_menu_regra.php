...
$titulo="Cadastro Regra de Menu para Templates";
$titulo_listar="Regras Já Cadastradas";
$id_item=$_GET['id_item'];
$arquivo="cad_template_menu_regra.php";
$tabela="sgc_template_regra";
$id_chave="id_template_regra";
...
<form method="POST" id="form1" action="sgc.php?action=cad_template_menu_regra.php&acao_int=select_menu" style="font-family: Verdana; font-size: 8pt">
					...
					<table border="0" width="488" cellspacing="0" cellpadding="0">
						<tr>
                            ...
                            <select size="1" name="usuario_menu"  onchange="this.form.submit();" style=" <?echo $borda?> font-family: Verdana, arial, helvetica, sans-serif; font-size: 11px; color: #000000;  background: #FFFFFF;"  >
                                 ...
                                 <?
                          $checa = mysql_query("select id_template,descricao from sgc_template_menu order by descricao desc ") or print(mysql_error());
                                    while($dados=mysql_fetch_array($checa)){
                                    $id_objeto = $dados['id_template'];
...
<script language="javascript"  src="ajax-template-menu.js" type="text/javascript"></script>
...
<form method="POST" id="form1" name='meuFormulario' action="sgc.php?action=cad_template_menu_regra.php&acao_int=cad_bd_regra" onsubmit='seleciona();loopSelectedAS()' style="font-family: Verdana; font-size: 8pt">
...
<div align="center">
	<table class="border" cellSpacing="0" cellPadding="0" width="100%" border="0">
		<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" border="0">
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
					<td class="info" align="middle"><b>:: Adicionar Item ao <?echo $titulo?> :: </b></td>
				</tr>
				<tr style="color: #000000; font-family: Arial, Helvetica, sans-serif; font-size: 12px">
					<td class="cat" align="center">
					<table border="0" width="488" cellspacing="0" cellpadding="0">
                        <tr>
							...
							<font color="#FF0000"><?echo $msg?></font></td>
                            <input type='hidden' name='id_item' value='<?echo $id_item?>'></td>
							<td height="23" width="418"><?echo tabelainfo($usuario_menu,'sgc_template_menu','descricao','id_template')?> </td>
						</tr>
                        <tr>
							<td height="23" width="66">
							<p align="right">Menu:&nbsp;
                            ...
                            <select size="1" name="menu"  Onchange="atualiza(this.value);"  style=" <?echo $borda?> font-family: Verdana, arial, helvetica, sans-serif; font-size: 11px; color: #000000;  background: #FFFFFF;"  >
                                 ...
                                 <?
                         $checa = mysql_query("select mn.id_menu, mn.descricao from  sgc_menu mn") or print(mysql_error());
                                    while($dados=mysql_fetch_array($checa)){
...
$deleta = mysql_query("DELETE FROM sgc_template_regra where id_template=$usuario and id_menu=$menu ") or print(mysql_error());

foreach($vetor_conjunto as $value){
 echo "Item:$value <br>";

  $checa = mysql_query("select * from sgc_template_regra where id_menu=$menu and id_item_menu=$value and id_template=$usuario") or $myerr = mysql_error();
  $row = mysql_num_rows($checa);

  if($row<1){
     $cadas = mysql_query("INSERT INTO sgc_template_regra (id_template,id_menu,id_item,data_criacao,quem_criou)
                         VALUES ($usuario,$menu,$value,sysdate(),$idusuario)") or print(mysql_error());
  }
 }
header("Location: ?action=cad_template_menu_regra.php&acao_int=select_menu&id_item=$id_item&id_usuario=$usuario");
}
