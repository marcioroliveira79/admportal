...
SQL="select ip_host from sgc_servidores"
ips=$(mysql --user=${USER_NAME} --password=${PASSWORD} ${DATABASE} -h ${IP_ADDR} -e "$SQL")
...
echo ${resul_erro[1]}
  ...
  if [ $existe_erro == "ok" ];then
	  ...
	  comando=$(mysql --user=${USER_NAME} --password=${PASSWORD} ${DATABASE} -h ${IP_ADDR} -e "UPDATE sgc_servidores SET erro_status='OFF', arquivo_erro=null  WHERE ip_host = '$i'")
  fi
      echo "------------------------"  
