...
if (isset($_SESSION['global_id_usuario']) && !empty($_SESSION['global_id_usuario']) && $acao != null) {
    ...
    if ($acesso == "TELA AUTORIZADA") {
        ...
        // Filtro: menu escolhido no combo (GET)
        $menu_filter = isset($_GET['menu_filter']) ? (int)$_GET['menu_filter'] : 0;
        // Paginação: página atual e limite
        $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
        $limit = 5; // Quantidade de itens por página (alterado para 5)
        $offset = ($page - 1) * $limit;

        // Monta a query considerando o filtro
        $where_clause = "";
        $params = [];
        if ($menu_filter > 0) {
        ...
        // Primeiro, conte o total de registros para a paginação
        ...
        // Agora, busque os registros com limite e offset
